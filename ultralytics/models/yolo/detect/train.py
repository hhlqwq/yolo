...
class DetectionTrainer(BaseTrainer):
    ...
    def build_dataset(self, img_path: str, mode: str = "train", batch: int | None = None):
        ...
        rectangular_size = isinstance(self.args.imgsz, (list, tuple))
        return build_yolo_dataset(
            ...
            self.data,
            mode=mode,
            rect=mode == "val" and not rectangular_size,
            stride=gs,
        )
    ...
    def preprocess_batch(self, batch: dict) -> dict:
        ...
        if self.args.multi_scale > 0.0:
            imgs = batch["img"]
            base_max = max(self.args.imgsz) if isinstance(self.args.imgsz, (list, tuple)) else self.args.imgsz
            sz = random.randrange(
                max(self.stride, int(base_max * (1.0 - self.args.multi_scale))),
                int(base_max * (1.0 + self.args.multi_scale) + self.stride),
