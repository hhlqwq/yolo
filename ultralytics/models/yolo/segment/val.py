...
class SegmentationValidator(DetectionValidator):
    ...
    def preprocess(self, batch: dict[str, Any]) -> dict[str, Any]:
        ...
        # Segment prototypes are normally stride 4, but heads that start at P2 produce stride-2 prototypes.
        # Keep the real input shape so post-processing never has to infer it from the prototype resolution.
    ...
    def postprocess(self, preds: list[torch.Tensor]) -> list[dict[str, torch.Tensor]]:
        ...
        self.mask_shape = tuple(proto.shape[2:])
        # `proto * 4` is only valid for the standard P3 segmentation head. P2 segmentation heads use a
        # higher-resolution prototype, so use the actual preprocessed image shape captured from the batch.
        imgsz = getattr(self, "batch_imgsz", tuple(4 * x for x in proto.shape[2:]))
    ...
    def _prepare_batch(self, si: int, batch: dict[str, Any]) -> dict[str, Any]:
        ...
        if nl:
            mask_size = (
                ...
                else getattr(self, "mask_shape", tuple(s // 4 for s in prepared_batch["imgsz"]))
            )
