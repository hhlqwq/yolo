...
class BaseTrainer:
    ...
    def _setup_train(self):
        ...
        rectangular_imgsz = isinstance(self.args.imgsz, (list, tuple))
        if rectangular_imgsz and self.args.task not in {"detect", "segment"}:
            raise ValueError(f"task={self.args.task} 暂不支持矩形 imgsz，请传入单个整数。")
        self.args.imgsz = check_imgsz(
            self.args.imgsz,
            stride=gs,
            floor=gs,
            max_dim=2 if rectangular_imgsz else 1,
            min_dim=2 if rectangular_imgsz else 1,
        )
    ...
    def _do_train(self):
        ...
        while True:
                    ...
                    pbar.set_description(
                        ...
                        % (
                            ...
                            *self.tloss.values(),  # losses
                            ...
                            "x".join(map(str, batch["img"].shape[-2:])),
                        )
                    )
            ...
            self.log_pipeline_epoch_summary()
            if self.args.time:
    ...
    def auto_batch(self, max_num_obj=0, dataset_size=0):
        ...
        max_imgsz = (
            [math.ceil(size * (1 + self.args.multi_scale) / self.stride) * self.stride for size in self.args.imgsz]
            if isinstance(self.args.imgsz, (list, tuple))
            else math.ceil(self.args.imgsz * (1 + self.args.multi_scale) / self.stride) * self.stride
        )
    ...
    def log_pipeline_epoch_summary(self):
        """由数据流水线启动训练时,输出一条干净的 Epoch 汇总."""
        if os.environ.get("PIPELINE_SINGLE_LOG") != "1" or RANK not in {-1, 0}:
            return
        values = {**self.label_loss_items(self.tloss), **self.metrics, **self.lr}
        fields = []
        ...
        fitness = "None" if self.fitness is None else f"{float(self.fitness):.6f}"
        LOGGER.info(
            "YOLO Epoch Summary | epoch=%d/%d | time=%.1fs | fitness=%s | %s",
            self.epoch + 1,
            self.epochs,
            self.epoch_time,
            fitness,
    ...
    def final_eval(self):
        ...
        self._best_epoch_before_strip = None
        self._best_fitness_before_strip = None
        self._best_train_metrics_before_strip = None
        with torch_distributed_zero_first(LOCAL_RANK):  # strip only on GPU 0; other GPUs should wait
            if RANK in {-1, 0}:
                if model:
                    try:
                        best_checkpoint, _ = torch_safe_load(self.best)
                        checkpoint_epoch = int(best_checkpoint.get("epoch", -1))
                        if checkpoint_epoch >= 0:
                            self._best_epoch_before_strip = checkpoint_epoch + 1
                        self._best_fitness_before_strip = best_checkpoint.get("best_fitness")
                        self._best_train_metrics_before_strip = best_checkpoint.get("train_metrics")
                    except Exception as exc:
                        LOGGER.warning("清理 best.pt 前读取最佳训练记录失败:%s", exc)
                ...
                if model:
                    ...
                    strip_optimizer(
                        ...
                        updates={
                            ...
                            "best_epoch": self._best_epoch_before_strip,
                            "best_fitness_value": self._best_fitness_before_strip,
                            "train_metrics": self._best_train_metrics_before_strip,
                        },
                    )
        if model:
            LOGGER.info(f"\nValidating {model}...")
    ...
    def infer_best_epoch_from_results(self, metrics: dict) -> int | None:
        """通过 results.csv 中与最佳 checkpoint 指标最接近的行恢复真实最佳 Epoch."""
        results = self.read_results_csv()
        if not results:
            return None
        columns = {str(name).strip(): values for name, values in results.items()}
        epochs = columns.get("epoch")
        if not epochs:
            return None
        metric_names = [
            name for name, value in metrics.items()
            if name != "fitness" and name in columns and isinstance(value, (int, float, np.number))
        ]
        if not metric_names:
            return None
        scores = []
        for index in range(len(epochs)):
            ...
            for name in metric_names:
                try:
                    score += (float(columns[name][index]) - float(metrics[name])) ** 2
                except (IndexError, TypeError, ValueError):
                    valid = False
                    break
            scores.append(score if valid else float("inf"))
        best_index = int(np.argmin(scores))
        if not np.isfinite(scores[best_index]):
            return None
        return int(round(float(epochs[best_index])))

    def log_best_model_metrics(self):
        """输出 best.pt 对应的最佳 epoch、fitness 和该 epoch 的验证指标."""
        if not self.best.exists():
            LOGGER.warning("未找到 best.pt,无法输出最佳 epoch 和验证指标.")
            return

        try:
            checkpoint, _ = torch_safe_load(self.best)
            metrics = self._best_train_metrics_before_strip or checkpoint.get("train_metrics", {})
            if not isinstance(metrics, dict):
                metrics = {}
            best_epoch = self.infer_best_epoch_from_results(metrics)
            if best_epoch is not None:
                LOGGER.info("已根据 results.csv 确认 best.pt 对应的真实最佳 Epoch:%d", best_epoch)
            else:
                best_epoch = self._best_epoch_before_strip or checkpoint.get("best_epoch")
            if best_epoch is None:
                checkpoint_epoch = int(checkpoint.get("epoch", -1))
                best_epoch = checkpoint_epoch + 1 if checkpoint_epoch >= 0 else None
            best_fitness = self._best_fitness_before_strip
            if best_fitness is None:
                best_fitness = checkpoint.get("best_fitness_value")
            if best_fitness is None:
            ...
            metric_text = ", ".join(
                ...
                if isinstance(value, (int, float, np.number)) and name != "fitness"
            )
            fitness_text = "N/A" if best_fitness is None else f"{float(best_fitness):.5f}"
            if best_epoch is None:
                LOGGER.warning("无法恢复 best.pt 对应的真实最佳 Epoch,将在结果中留空.")
            else:
                LOGGER.info(
                    "\n最佳模型结果:best.pt 来自 epoch %d,fitness=%s%s",
                    best_epoch,
                    fitness_text,
                    f",验证指标:{metric_text}" if metric_text else "",
                )
            class_metrics = []
            validator_metrics = getattr(self.validator, "metrics", None)
            if validator_metrics is not None and hasattr(validator_metrics, "summary"):
                class_metrics = validator_metrics.summary()
                if hasattr(validator_metrics, "seg"):
                    for index, item in enumerate(class_metrics):
                        mask_result = validator_metrics.seg.class_result(index)
                        item["Mask-mAP50"] = float(mask_result[2])
                        item["Mask-mAP50-95"] = float(mask_result[3])
            summary_path = self.save_dir / "best_metrics.json"
            ...
            LOGGER.info("最佳模型逐类别指标已保存:%s", summary_path)
        except Exception as exc:
            LOGGER.warning("无法读取 best.pt 的最佳训练结果:%s", exc)
