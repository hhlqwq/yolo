...
class YOLODataset(BaseDataset):
    ...
    def get_labels(self) -> list[dict]:
        ...
        """
        labels_dir = self.data.get("labels_dir", "labels")
        if isinstance(labels_dir, dict):
            labels_dir = labels_dir.get(self.task, labels_dir.get("default"))
            if labels_dir is None:
                raise KeyError(
                    f"'labels_dir' has no entry for task '{self.task}'. "
                    "Add that task or a 'default' entry to the dataset YAML."
                )
        if not isinstance(labels_dir, (str, Path)):
            raise TypeError(f"'labels_dir' must be a path string or task mapping, got {type(labels_dir).__name__}")
        self.label_files = img2label_paths(self.im_files, label_dir=str(labels_dir))
    ...
    def build_transforms(self, hyp: dict | None = None) -> Compose:
        ...
        if self.augment:
            ...
            shape = (self.imgsz, self.imgsz) if isinstance(self.imgsz, int) else tuple(self.imgsz)
            transforms = Compose([LetterBox(new_shape=shape, scaleup=False)])
        transforms.append(
            Format(
                bbox_format="xywh",
                normalize=True,
