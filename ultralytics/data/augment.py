...
class Mosaic(BaseMixTransform):
    ...
    def __init__(self, dataset, imgsz: int | list[int] | tuple[int, int] = 640, p: float = 1.0, n: int = 4):
        ...
        Args:
            ...
            imgsz (int | list[int] | tuple[int, int]): Image size as an integer or (height, width).
            p (float): Probability of applying the mosaic augmentation. Must be in the range 0-1.
            n (int): The grid size, either 4 (for 2x2) or 9 (for 3x3).
        """
        ...
        self.imgsz = imgsz
        self.height, self.width = (imgsz, imgsz) if isinstance(imgsz, int) else imgsz
        self.border = (-self.height // 2, -self.width // 2)  # height, width
        self.n = n
    ...
    def get_params(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        layout = []
        if self.n == 4:
            yc = int(random.uniform(-self.border[0], 2 * height + self.border[0]))
            xc = int(random.uniform(-self.border[1], 2 * width + self.border[1]))
            for i in range(4):
            ...
            for i in range(9):
                ...
                h, w = labels_patch.get("resized_shape", img.shape[:2])
                if i == 0:  # center
                    c = width, height, width + w, height + h
                    h0, w0 = h, w
                elif i == 1:  # top
                    c = width, height - h, width + w, height
                elif i == 2:  # top right
                    c = width + wp, height - h, width + wp + w, height
                elif i == 3:  # right
                    c = width + w0, height, width + w0 + w, height + h
                elif i == 4:  # bottom right
                    c = width + w0, height + hp, width + w0 + w, height + hp + h
                elif i == 5:  # bottom
                    c = width + w0 - w, height + h0, width + w0, height + h0 + h
                elif i == 6:  # bottom left
                    c = width + w0 - wp - w, height + h0, width + w0 - wp, height + h0 + h
                elif i == 7:  # left
                    c = width - w, height + h0 - h, width, height + h0
                elif i == 8:  # top left
                    c = width - w, height + h0 - hp - h, width, height + h0 - hp
                padw, padh = c[:2]
    ...
    def apply_image(self, labels: dict[str, Any], params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if self.n == 4:
            img4 = np.full((self.height * 2, self.width * 2, labels["img"].shape[2]), 114, dtype=np.uint8)
            for item in layout:
            ...
            img9 = np.full((self.height * 3, self.width * 3, labels["img"].shape[2]), 114, dtype=np.uint8)
            for item in layout:
                labels_patch = item["labels_patch"]
                img = labels_patch["img"]
    ...
    def apply_semantic(self, labels: dict[str, Any], params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        layout = params["layout"]
        if self.n == 4:
            mask4 = np.full((self.height * 2, self.width * 2), 255, dtype=np.uint8)
            for item in layout:
                labels_patch = item["labels_patch"]
            ...
            mask9 = np.full((self.height * 3, self.width * 3), 255, dtype=np.uint8)
            for item in layout:
                labels_patch = item["labels_patch"]
                mask = labels_patch.get("semantic_mask")
                if mask is None:
    ...
    def _cat_labels(self, mosaic_labels: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        final_labels["instances"].clip(width, height, preserve_obb=self.preserve_obb)
...
def v8_transforms(
    dataset, imgsz: int | list[int] | tuple[int, int], hyp: IterableSimpleNamespace, stretch: bool = False
):
    ...
    includes operations such as mosaic, copy-paste, random perspective, mixup, and various color adjustments.
        ...
        Args:
            ...
            imgsz (int | list[int] | tuple[int, int]): The target image size as an integer or (height, width).
            hyp (IterableSimpleNamespace): A namespace of hyperparameters controlling various aspects of the
                transformations.
            stretch (bool): 是否为拉伸预处理保留未指定的仿射输出尺寸。
    ...
    affine = RandomPerspective(
        ...
        size=(width, height) if not stretch else None,
