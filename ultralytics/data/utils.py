...
def img2label_paths(img_paths: list[str], label_dir: str = "labels", suffix: str = ".txt") -> list[str]:
    """Convert image paths to label paths using a sibling or absolute label directory.

    Args:
        img_paths (list[str]): Image file paths containing an ``images`` directory component.
        label_dir (str): Label root. Relative paths replace the sibling ``images`` directory, while absolute paths
            preserve the portion below ``images`` beneath the supplied root.
        suffix (str): Label file suffix.

    Examples:
        >>> img2label_paths(["dataset/images/train/im.jpg"], "labels_detect")
        ['dataset/labels_detect/train/im.txt']
    """
    label_root = Path(label_dir)
    if label_root.is_absolute():
        labels = []
        for image in map(Path, img_paths):
            parts = image.parts
            image_index = next((i for i in range(len(parts) - 1, -1, -1) if parts[i].lower() == "images"), None)
            relative = Path(*parts[image_index + 1 :]) if image_index is not None else Path(image.name)
            labels.append(str((label_root / relative).with_suffix(suffix)))
        return labels

    sa = f"{os.sep}images{os.sep}"
    sb = f"{os.sep}{label_root}{os.sep}"
