...
class BaseDataset(Dataset):
    ...
    def load_image(
        ...
        if im is None:  # not cached in RAM
            ...
            target_h, target_w = (self.imgsz, self.imgsz) if isinstance(self.imgsz, int) else self.imgsz
            if isinstance(self.imgsz, (list, tuple)):
                r = max(target_h / h0, target_w / w0) if resize_short else min(target_h / h0, target_w / w0)
                if r != 1:
    ...
    def set_rectangle(self) -> None:
        ...
        if isinstance(self.imgsz, (list, tuple)):
            self.batch_shapes = np.tile(np.asarray(self.imgsz, dtype=int), (nb, 1))
            self.batch = bi
            return
