...
def test_task_specific_label_directories(tmp_path):
    """Test detection and segmentation labels sharing one image directory with isolated caches."""
    from ultralytics.data.dataset import YOLODataset

    root = tmp_path / "shared"
    image_dir = root / "images" / "train"
    detect_dir = root / "labels_detect" / "train"
    segment_dir = root / "labels_segment" / "train"
    for directory in (image_dir, detect_dir, segment_dir):
        directory.mkdir(parents=True)

    Image.new("RGB", (32, 32)).save(image_dir / "sample.jpg")
    (detect_dir / "sample.txt").write_text("0 0.5 0.5 0.5 0.5\n")
    (segment_dir / "sample.txt").write_text("0 0.25 0.25 0.75 0.25 0.75 0.75 0.25 0.75\n")
    data = {
        "names": {0: "item"},
        "labels_dir": {"detect": "labels_detect", "segment": "labels_segment"},
    }
    kwargs = {
    ...
    detect_dataset = YOLODataset(task="detect", **kwargs)
    segment_dataset = YOLODataset(task="segment", **kwargs)

    assert Path(detect_dataset.label_files[0]) == detect_dir / "sample.txt"
    assert Path(segment_dataset.label_files[0]) == segment_dir / "sample.txt"
    assert not detect_dataset.labels[0]["segments"]
    assert len(segment_dataset.labels[0]["segments"]) == 1
    assert (root / "labels_detect" / "train.cache").is_file()
    assert (root / "labels_segment" / "train.cache").is_file()
